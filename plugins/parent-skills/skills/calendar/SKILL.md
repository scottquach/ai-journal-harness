...
Use the Google Calendar domain tools to create, update, delete, and answer questions about events, schedule, time windows, and availability. If only `getCalendarEvents` is available (iCal fallback), use it only for read-only event lookup.

When backed by Google Calendar, available tools are: `getCalendarEvents`, `listCalendarEvents`, `listCalendars`, `createCalendarEvent`, `updateCalendarEvent`, `deleteCalendarEvent`, `findCalendarEvent`, `findCalendarFreeSlots`.

When backed by the iCal fallback, only `getCalendarEvents` is available and it is read-only.
...
2. Use `createCalendarEvent` if it is exposed in your active tool list.
3. If the user does not give a duration, default to 30 minutes.
4. If the user says "no reminder" or "no reminders", disable reminders/notifications.
...
Calendar tools appear directly in your active tool list when the runtime is configured. If no write tools appear when this skill runs, report immediately that calendar writes are unavailable in this session and stop.
